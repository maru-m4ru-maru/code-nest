// Code Nest runtime loader V0.4.2
(() => {
  'use strict';
  ...
  async function runShareDirect(button) {
    ...
    console.log('[Code Nest Share V0.4.2] START');

    try {
      const data = shareSnapshot();
      console.log('[Code Nest Share V0.4.2] SNAPSHOT', { title: data.title, cells: data.cells.length });
      ...
      console.log('[Code Nest Share V0.4.2] CREATED', base);

      document.querySelector('#shareV42Result')?.remove();
      const box = document.createElement('div');
      box.id = 'shareV42Result';
      ...
      box.innerHTML = `<div style="width:min(640px,100%);background:#fff;color:#111827;border-radius:20px;padding:22px;box-shadow:0 24px 90px rgba(0,0,0,.3)">
        <div style="display:flex;justify-content:space-between;align-items:center;gap:12px"><strong style="font-size:20px">Share your project</strong><button type="button" id="shareV42Close" style="border:0;background:none;font-size:26px;cursor:pointer">×</button></div>
        ...
        <div style="display:flex;gap:8px;justify-content:flex-end;margin-top:16px"><button type="button" id="shareV42Code" style="padding:9px 12px;border:1px solid #ddd;border-radius:10px;background:#fff;cursor:pointer">コードを開く</button><button type="button" id="shareV42Preview" style="padding:9px 12px;border:1px solid #ddd;border-radius:10px;background:#fff;cursor:pointer">プレビューを開く</button></div>
      </div>`;
      document.body.appendChild(box);
      box.querySelector('#shareV42Close').onclick = () => box.remove();
      box.querySelector('#shareV42Code').onclick = () => window.open(codeUrl, '_blank', 'noopener,noreferrer');
      box.querySelector('#shareV42Preview').onclick = () => window.open(previewUrl, '_blank', 'noopener,noreferrer');
      box.addEventListener('click', (event) => { if (event.target === box) box.remove(); });
      button.textContent = '✓ 共有済み';
    } catch (error) {
      console.error('[Code Nest Share V0.4.2] FAILED', error);
      alert(`共有に失敗しました\n${error?.message || error}`);
  ...
  document.addEventListener('click', (event) => {
    ...
    if (!button) return;
    console.log('[Code Nest Share V0.4.2] CLICK CAPTURE', { disabled: button.disabled });
  ...
  console.log('[Code Nest] runtime loader V0.4.2 ready');
})();
