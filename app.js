...
function previewAssetCell(cell,kind){const src=cell.querySelector('textarea').value;const frame=$('#previewFrame');let html=getCellSourceByExt('.html');if(!html)html='<!doctype html><html><head><meta charset="utf-8"></head><body><h2>Code Nest preview</h2><p>HTMLセルを追加すると、そこへCSS/JSを組み合わせます。</p></body></html>';if(!/<html\b/i.test(html))html='<!doctype html><html><head><meta charset="utf-8"></head><body>'+html+'</body></html>';if(kind==='css')html=html.replace('</head>','<style>'+src+'</style></head>');else html=html.replace('</body>','<script>'+src+'<\\/script></body>');const blob=new Blob([html],{type:'text/html'});const u=URL.createObjectURL(blob);frame.src=u;$('#previewLabel').textContent=kind==='css'?'CSSセルのブラウザプレビュー':'JavaScriptセルのブラウザプレビュー';openModal('#previewModal');$('#previewNewTab').onclick=()=>window.open(u,'_blank','noopener');frame.dataset.previewUrl=u}
